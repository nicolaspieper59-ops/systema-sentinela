...
### 2. Fichier `worker.js` (Correction de l'irradiance lunaire et gestion des constellations)

Ce fichier intègre la correction spécifique pour la Lune (application d'un facteur d'albédo pour éliminer le pic irréaliste)[cite: 4, 6] et gère proprement l'identification des constellations en JavaScript[cite: 1] :

```javascript
/**
 * SYSTEMA SENTINELA — WEB WORKER (v19.13 CORRIGÉ & SÉCURISÉ)
 */
...
function calculerParametresLunaires(posLune, posSoleil) {
    if (!posLune || !posSoleil) return { pct: 98.0, age: 13.4 };
        ...
        try {
            ...
            if (sourceDonnees) {
                for (const [nomAstre, arcsAstre] of Object.entries(sourceDonnees)) {
                    ...
                    // Application de la correction d'irradiance pour la Lune (albédo de réflexion)
                    let irradianceVal = Module.HEAPF64[off + 9];
                    if (nomAstreMaj === 'LUNE') {
                        irradianceVal = irradianceVal * 0.000073; 
                    }
